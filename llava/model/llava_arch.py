...
def phase_select_layer(mm_vision_select_layer):
    if isinstance(mm_vision_select_layer, int) or isinstance(mm_vision_select_layer, list):
        return mm_vision_select_layer
    try:
        if mm_vision_select_layer[:4] == "list":
            # list开头的话，就传进来的是list
            mm_vision_select_layer = [int(layer.strip()) for layer in mm_vision_select_layer[4:].split("/")]
        else:
            mm_vision_select_layer = int(mm_vision_select_layer)
        print(f"[phase_select_layer] mm_vision_select_layer: {mm_vision_select_layer}")

        return mm_vision_select_layer
    except Exception:
        raise ValueError(f"Error in mm_vision_select_layer {mm_vision_select_layer}")
...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):  # for eval
            config.mm_vision_select_layer = phase_select_layer(config.mm_vision_select_layer)
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        model_args.mm_vision_select_layer = phase_select_layer(model_args.mm_vision_select_layer)  # phase mm_vision_select_layer
        ...
        if pretrain_vision_tower is not None:
            print("pretrain_vision_tower is not None")
            ...
            print('****** get_w', get_w(vision_tower_weights, 'vision_tower'))
            self.vision_tower.load_state_dict(get_w(vision_tower_weights, 'vision_tower'))
